...
class ModuleHtmlPlugin {
  ...
  apply (compiler) {
    ...
    compiler.hooks.compilation.tap(id, async compilation => {
      // 在 htmlWebpackPlugin 拿到资源的时候我们处理下
      await compilation.hooks.htmlWebpackPluginAlterAssetTags.tapAsync(
        id,
        (data, cb) => {
          data.body.forEach(tag => {
            //遍历下资源，把 script 中的 ES2016 和 es15 的处理开
            if (tag.tagName === 'script') {
              // 给 es15 的资源加上 nomodule 属性，反之加上 type="module" 的属性
              console.log(tag)
              if (/.es15./.test(tag.attributes.src)) {
